...
function AmenityCard({ item }) {
  ...
  return (
    <div className="flex items-center justify-between p-5 border border-gray-100 hover:border-gray-200 transition-all bg-white mb-2">
...
const StationDetail = () => {
  ...
  const fetchChargers = async () => {
    try {
      const { data, error } = await supabase.from('chargers').select('*').eq('station_id', id)
  ...
  const fetchReviews = async () => {
    try {
      const { data, error } = await supabase.from('reviews').select('*').eq('station_id', id).order('created_at', { ascending: false })
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'chargers', filter: `station_id=eq.${id}` }, () => loadData())
      .subscribe()
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900 antialiased">
      ...
      {/* SECTION 1 - Top Bar */}
      <div className="border-b border-gray-100 bg-white sticky top-[72px] lg:top-[80px] z-40">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <button
            onClick={() => navigate(-1)}
            className="text-sm text-gray-400 hover:text-gray-600 transition-all flex items-center gap-1.5 font-medium">
            ← Back to stations
          </button>
          <div className="flex items-center gap-6">
            <button 
              onClick={handleShare}
              className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1.5 transition-all font-medium">
              <Share2 size={14} />
              Share
            </button>
            <button 
              onClick={toggleSave}
              className={`text-xs flex items-center gap-1.5 transition-all font-medium ${isSaved ? 'text-gray-900 font-semibold' : 'text-gray-400 hover:text-gray-600'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-7">
            ...
            <div className="mb-12">
              ...
              <h1 className="text-3xl font-semibold text-gray-900 mb-5 leading-snug tracking-tight">
                {station.name}
              </h1>
                  ...
                  <div className="flex items-center -space-x-1">
                    {[1,2,3,4,5].map(star => (
                      <Star key={star} size={13} 
                        className={star <= Math.round(avgRating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-100'} 
                      />
                    ))}
                  </div>
              ...
              {/* QUICK INFO PILLS - FIX 4 */}
              <div className="flex flex-wrap gap-2">
                <span className="text-xs border border-gray-200 px-3 py-1.5 text-gray-500 font-normal flex items-center">
                  <div className="w-1.5 h-1.5 rounded-full bg-gray-400 mr-2"></div>
                  {availableCount}/{chargers.length} available
                </span>
                <span className="text-xs border border-gray-200 px-3 py-1.5 text-gray-500 font-normal">
                  Starting ₹{minPrice}/kWh
                </span>
                <span className="text-xs border border-gray-200 px-3 py-1.5 text-gray-500 font-normal uppercase tracking-tight">
                  AQI {aqiLevel}
                </span>
              </div>
            </div>
            ...
            {/* TABS - FIX 5 */}
            <div className="border-b border-gray-100 mb-12 overflow-x-auto">
              <div className="flex min-w-max">
                {TABS.map(tab => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`text-sm font-medium px-5 py-4 border-b-2 transition-all -mb-px ${
                    activeTab === tab
                    ...
                    {(tab === 'Chargers' || tab === 'Reviews') && (
                      <span className={`ml-1.5 text-[10px] px-1.5 py-0.5 font-medium leading-none ${
                        activeTab === tab
              ...
              {activeTab === 'Chargers' && (
                <div className="space-y-6">
                  {chargers.map(charger => (
                    <div key={charger.id} className="border border-gray-100 p-8 hover:border-gray-200 transition-all bg-white group">
                      {/* CHARGER HEADER - FIX 6 */}
                      ...
                      {/* CHARGER SPEC TAGS - FIX 7 */}
                      <div className="flex gap-2 mb-8">
                        <span className="text-xs border border-gray-200 px-3 py-1.5 text-gray-500 font-medium">
                          {charger.power_kw} kW
                        </span>
                        <span className="text-xs border border-gray-200 px-3 py-1.5 text-gray-500 font-medium capitalize">
                          {charger.plug_type || charger.type}
                        </span>
                        <span className="text-xs border border-gray-200 px-3 py-1.5 text-gray-500 font-medium">
                      ...
                      <div className="mb-10">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[10px] font-medium text-gray-400">Activity Level</span>
                          <span className="text-[10px] font-medium text-gray-400">{charger.sessions_today || 0} usage sessions</span>
                        </div>
                        <div className="h-px bg-gray-50 overflow-hidden">
                          <div className="h-full bg-gray-300 transition-all duration-1000" style={{ width: `${Math.min(100, (charger.sessions_today/20)*100)}%` }}></div>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        {charger.status === 'available' ? 'Book This Charger' : 'Currently Unavailable'}
                      </button>
                    </div>
                  ))}
                </div>
              ...
              {activeTab === 'Facilities' && (() => {
                ...
                return (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {facilities.map(f => (
                      <div key={f} className="border border-gray-50 p-6 flex flex-col gap-4 bg-white hover:border-gray-100 transition-all group">
          ...
          <div className="lg:col-span-5 lg:pl-12">
            <div className="sticky top-24">
              ...
              <button
                onClick={() => navigate(`/book/${id}`)}
                className="w-full bg-gray-900 text-white py-4 text-sm font-medium hover:bg-black transition-all mb-2">
              ...
              <div className="mb-10">
                ...
                <div className="h-px bg-gray-100 my-4">
                  <div className="h-full bg-gray-400 transition-all duration-700"
                    style={{width: `${availablePct}%`}}>
                  </div>
                </div>
                
                <p className="text-[10px] text-gray-400 uppercase tracking-tight">
                  {occupiedPct}% terminal load
                </p>
              </div>
              ...
              {/* ATMOSPHERIC LIVE DATA section - FIX 10 */}
              <div className="mb-10">
              ...
              {/* STATION DETAILS section - FIX 11 */}
              <div className="mb-10">
                <p className="text-xs text-gray-400 mb-6">
                  Station details
                </p>
      ...
      <section className="bg-gray-50 border-t border-gray-100 py-16">
          <div className="max-w-3xl mx-auto px-6 text-center">
              ...
              <button onClick={() => navigate(`/book/${id}`)} className="bg-gray-900 text-white px-10 py-4 text-xs font-medium hover:bg-black transition-all">Start Booking Session</button>
          </div>
      </section>
    </div>
  )
